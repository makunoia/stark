...
const IconButton = ({
  ...
  fullWidth = false,
  attached = false,
  //set true if button is attached to a field
  onClick,
  className
}) => {
  const type = outline ? "outline" : text ? "text" : "solid"; // Set solid as the default
  const size = small ? "small" : large ? "large" : "default";
  const color = primary ? "primary" : secondary ? "secondary" : success ? "success" : info ? "info" : danger ? "danger" : warning ? "warning" : dark ? "dark" : "default";
  ...
  const typeClasses = {
    solid: `${attached ? "border-l rounded-0px" : "rounded-4px outline border"} focus:outline-4 focus:outline-focusRing/30 outline-focusRing/0`,
    outline: `${attached ? "border-l rounded-0px" : "rounded-4px outline border"} focus:outline-4 focus:outline-focusRing/30 outline-focusRing/0`,
  ...
  return /*#__PURE__*/(0, _jsxRuntime.jsxs)("button", {
    ...
    className: (0, _clsx.default)(`relative group justify-center items-center flex font-default transition-colors duration-200 ease-in-out h-fit ${typeClasses[type]} ${fullWidth ? "w-full" : "w-fit"} ${type !== "text" ? sizeClasses[size] : ""} ${state === "disabled" //check if the button is disabled
    ...
    } ${state !== "text" && state === "disabled" ? stateClasses["disabled"] : ""} ${className}
     `),
    children: [/*#__PURE__*/(0, _jsxRuntime.jsx)("label", {
      className: (0, _clsx.default)(`inline-flex gap-4px items-center justify-center font-semibold text-body whitespace-nowrap ${state !== "disabled" ? color === "default" ? textClasses["default"] : type === "solid" ? textClasses["white"] : textClasses[color] : textClasses["disabled"]} ${loading && "opacity-20 cursor-wait"}`),
      ...
      className: `absolute cursor-wait top-1/3 left-1/2 transform -translate-x-1/2 -translate-y-1/2 h-[18px] w-[18px] text-[18px] font-bold 
        ${color === "default" ? textClasses["default"] : type === "solid" ? textClasses["white"] : textClasses[color]}`,
      children: /*#__PURE__*/(0, _jsxRuntime.jsx)("div", {
